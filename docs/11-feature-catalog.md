...
| Popular-language catalog | Adds system-language selection plus 19 bundled languages: English, Spanish, Brazilian Portuguese, French, German, Italian, Polish, Ukrainian, Russian, Lithuanian, Japanese, Korean, Simplified Chinese, Hindi, Arabic, Dutch, Turkish, Indonesian, and Vietnamese. Navigation, route titles, locale-aware dates/times, and essential actions on Today, Focus, Capture, and Progress switch immediately. Android/iOS native builds advertise the supported locales, and Arabic enables RTL layout. Advanced copy without a reviewed translation falls back to English instead of becoming blank. Automated coverage requires every language to contain every core-experience key and checks Lithuanian copy, locale normalization, and RTL detection. | Bundled strings and local settings only: **$0** at 1, 1,000, 10,000, or 100,000 users. No translation API, account, analytics SDK, server, or network request. App bundle growth is the only footprint. |
